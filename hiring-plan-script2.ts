function main(workbook: ExcelScript.Workbook) {
  ...
  function createReasonChart() {
    ...
    } else if (REASON_TABLE.getRangeBetweenHeaderAndTotal().getUsedRange() === undefined) {
      throw new Error('There is no data filled per reason table. Paste your informations and run the script again')
    }
  ...
  function createDirectorChart() {
    let chartTitle: string = `Filled per Director - ${monthName[today.getMonth() - 1]}`

    if (PIVOT_SHEET.getChart(chartTitle)) {
      PIVOT_SHEET.getChart(chartTitle).delete()
    } else if (DIRECTOR_TABLE.getRangeBetweenHeaderAndTotal().getUsedRange() === undefined) {
      throw new Error('There is no data filled per director table. Paste your informations and run the script again')
    }
